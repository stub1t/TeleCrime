...
#   1. Pipeline subprocess hung (no DB activity + frozen progress + stale heartbeat)
...
# --- 3. Active DB query (pipeline doing real work)? ---
DB_ACTIVE=0
...
log "check: pipeline_pid=$PIPELINE_PID alive=$PIPELINE_ALIVE heartbeat_age=${HEARTBEAT_AGE}s db_active=$DB_ACTIVE"
